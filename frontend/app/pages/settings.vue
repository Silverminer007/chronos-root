...
onMounted(async () => {
  ...
  if (import.meta.client) {
    pushEnabled.value = isPushEnabled();
    pushAvailable.value = isPushAvailable();
  }
});
...
<template>
  <div v-if="pushAvailable" class="min-h-screen bg-gray-50 dark:bg-neutral-900">
    <SearchHeader/>
      ...
      <div class="max-w-4xl mx-auto">
        ...
        <div class="flex items-center justify-between mb-6">
          <div>
            ...
            <p class="text-gray-500 dark:text-gray-400 mt-1">
              Push Benachrichtigungen sind in deinem Browser nicht verfügbar
            </p>
          </div>
        </div>
      </div>
