<template>
  <div v-if="visible" class="fixed inset-0 flex items-center justify-center bg-black/60 z-50 p-4">
    <div class="w-full max-w-2xl bg-white dark:bg-neutral-800 rounded-xl shadow-2xl transform transition-all max-h-[90vh] flex flex-col">
      <!-- Header -->
      ...
      <div class="p-6 space-y-6 overflow-y-auto flex-1">
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
            {{ activeTab === 'user' ? 'Person suchen' : 'Gruppe suchen' }}
          </label>
          <div class="relative">
            ...
            <input
                v-model="searchQuery"
                type="text"
                :placeholder="activeTab === 'user' ? 'Name eingeben...' : 'Gruppenname eingeben...'"
          ...
          <div v-else class="space-y-2 max-h-60 overflow-y-auto">
            ...
            >
              <div
                  ...
                  : 'bg-linear-to-br from-blue-100 to-blue-200 dark:from-blue-900/30 dark:to-blue-800/30'"
              >
                <i
                    :class="activeTab === 'user' ? 'pi pi-user text-purple-600 dark:text-purple-400' : 'pi pi-users text-blue-600 dark:text-blue-400'"
                ></i>
              </div>
        ...
        <!-- Selected Item -->
        <div v-if="selectedItem" class="p-4 bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 rounded-lg">
...
const emit = defineEmits<{
  close: [];
  add: [data: { type: 'user' | 'group'; id: number; role: 'ATTENDANT' | 'RESPONSIBLE' | 'GUEST' }];
}>();

const {searchGroups, loading: loadingGroups} = useGroups();
const {searchContacts, loading: loadingContacts} = useContacts();

const searching = computed(() => {
  if(activeTab.value === 'user') {
    return loadingContacts.value;
  } else {
...
const selectedRole = ref<'ATTENDANT' | 'RESPONSIBLE' | 'GUEST'>('ATTENDANT');
