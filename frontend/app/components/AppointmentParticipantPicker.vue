...
import type {Group, Role, TeamMember} from '~/types';

type ParticipationRole = Exclude<Role, 'NONE'>

interface SelectedUser { id: string; name: string; role: ParticipationRole }
...
const allTeamMembers = computed<TeamMember[]>(() => {
...
const userResults = computed<TeamMember[]>(() => {
...
const isLoading = computed(() => tab.value === 'groups' && groupsLoading.value);
...
function onInput() {
  ...
  if (query.value.length < 2 || tab.value === 'users') {
    if (tab.value === 'groups') groupResults.value = [];
    return;
  }
...
function selectUser(member: TeamMember) {
  if (selectedUsers.value.some(u => u.id === member.userId)) return;
  selectedUsers.value.push({id: member.userId, name: memberName(member), role: 'ATTENDANT'});
  query.value = '';
}
  ...
  <div class="space-y-3">
    ...
    <div class="relative">
      ...
      <input
          v-model="query"
          type="text"
          :placeholder="tab === 'users' ? 'Teammitglied suchen…' : 'Gruppe suchen…'"
    ...
    >
      <!-- User (team member) results -->
      <template v-if="tab === 'users'">
        <button
            v-for="member in userResults"
            :key="member.userId"
            type="button"
            :disabled="selectedUsers.some(u => u.id === member.userId)"
        ...
        >
          <div class="w-8 h-8 rounded-full bg-linear-to-br from-purple-100 to-pink-100 dark:from-purple-900/30 dark:to-pink-900/30 flex items-center justify-center flex-shrink-0">
            <span class="text-purple-600 dark:text-purple-400 text-xs font-semibold">
              {{ member.firstName?.charAt(0)?.toUpperCase() || '?' }}
            </span>
          </div>
          <div class="flex-1 min-w-0">
            <p class="text-sm font-medium text-gray-900 dark:text-white truncate">{{ memberName(member) }}</p>
          </div>
          <Icon
              v-if="selectedUsers.some(u => u.id === member.userId)"
              name="lucide:check"
